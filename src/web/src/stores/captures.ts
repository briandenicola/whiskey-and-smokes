...
import { ref } from 'vue'
import { capturesApi, type CaptureResponse, type CreateCaptureRequest } from '../services/captures'
...
export const useCapturesStore = defineStore('captures', () => {
  ...
  async function createCapture(data: CreateCaptureRequest) {
    const response = await capturesApi.create(data)
    captures.value.unshift(response.data)
    return response.data
  }

  async function refreshCapture(id: string) {
